...
// (الأثاث/السلالم). صندوق واحد صلب بلا فتحات — قرار الإصدار الأول الموثّق بالخطة: الغرف
// الحرة جدرانها صلبة بس، بلا أبواب/نوافذ عليها. يُبنى داخل Three.Group بموضع منتصف الضلع
// ودوران مساوٍ لزاوية الضلع، فالصندوق الداخلي يبقى محاذي محلياً (نفس مبدأ WALL_T بالجدران
// العادية) بلا حاجة لأي حساب زوايا إضافي بعد هيك
function buildAngledWallSegment(group, animState, textureCache, wallColorHex, p1, p2, wallHeight, floorBaseY, center) {
  ...
  const mat = texturedMaterial(textureCache, wallColorHex, "wall", length, wallHeight, 0.85);
  addGrowBox(wallGroup, animState, WALL_T, wallHeight, length, 0, 0, 0, mat, WALL_DELAY);
}
...
// قشرة غرفة حرة الشكل (جدران مايلة) — جدار صلب لكل ضلع متتالٍ بالنقاط، أرضية وسقف بنفس
// شكل المضلع فعلياً. البلاطة الإنشائية دايماً تُبنى (نفس مبدأ buildRoomRoof: تمنع فجوة مع
// الطابق يلي فوق)، وحافة (parapet) حوالين كل ضلع بس لو has_roof مفعّل. خارج نطاق الإصدار
// الأول عن قصد: بلا أبواب/نوافذ/أثاث/دمج جدران مشتركة على الغرف الحرة (موثّق بالخطة)
function buildPolygonRoomShell(group, animState, textureCache, room, wallHeight, floorBaseY, wallColorHex, center, needsSlabForFloorAbove) {
  ...
  for (let i = 0; i < points.length; i++) {
    buildAngledWallSegment(group, animState, textureCache, wallColorHex, points[i], points[(i + 1) % points.length], wallHeight, floorBaseY, center);
  }
...
export function rebuildGroup(group, rooms, stairs, wallHeight, wallColor, center, animState, textureCache) {
  ...
  const topRoomFloor = Math.max(0, ...rooms.map((r) => r.floor ?? 0));
  ...
  rooms.forEach((r) => {
    ...
    if (r.points) {
      buildPolygonRoomShell(group, animState, textureCache, r, roomWallHeight, floorBaseY, roomWallColor, center, floorNum < topRoomFloor);
      return;
    }
    ...
    buildRoomRoof(group, animState, textureCache, r, roomWallHeight, floorBaseY, roomWallColor, cx, cz, !!r.has_roof, floorNum < topRoomFloor);
