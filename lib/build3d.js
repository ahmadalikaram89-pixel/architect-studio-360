...
import { speckleAmplitude, paintMaterialPattern, adjustRoughness } from "./materials";

/** @typedef {import("./types").Room} Room */
/** @typedef {import("./types").Point} Point */
/** @typedef {import("./types").Stair} Stair */

/**
 * @param {Pick<Room, "gx"|"gy"|"gw"|"gh"|"floor"|"wall_height">[]} rooms
 * @param {number} [wallHeight]
 * @returns {{ x: number, z: number, radius: number, targetY: number }}
 */
export function computeCenter(rooms, wallHeight = 2.7, floorGap = 0) {
  ...
  const floorBaseYMap = computeFloorBaseYMap(rooms, wallHeight, 0, floorGap);
...
/**
 * @param {Pick<Room, "floor"|"wall_height">[]} rooms
 * @param {number} defaultWallHeight
 * @param {number} [minTopFloor]
 * @param {number} [floorGap] فراغ إضافي بالمتر بين كل طابق ويلي فوقه — لعرض "بيت الدمية"
 *   بمبنى متعدد الطوابق (بلاطة الطابق الأعلى بتحجب الطابق تحتها تماماً من فوق). صفر =
 *   البناء الطبيعي الملتصق. **كل شي بالمشهد بيقرأ موضعه من هالخريطة** (جدران، أرضيات،
 *   أثاث، سلالم) — فتغيير رقم واحد هون بيفصل الطوابق كلها متسقة بلا أي كود تاني
 * @returns {Map<number, number>}
 */
export function computeFloorBaseYMap(rooms, defaultWallHeight, minTopFloor = 0, floorGap = 0) {
...
/**
 * @param {{ points?: Point[], gw?: number, gh?: number, [key: string]: any }} room
 * @returns {number}
 */
...
export const WIN_SILL = 0.9;
const WIN_MARGIN = 0.4;
const WIN_MIN_GAP = 0.6;
export const ROOF_T = 0.18;
...
function makeSpeckleTexture(hex, kind, material = "plaster") {
  const size = 256;
  ...
  const amplitude = speckleAmplitude(kind, material);
  ...
  paintMaterialPattern(ctx, hexStr, material, size);

  if (kind === "roof" && material === "plaster") {
...
function getBaseTexture(textureCache, hex, kind, material = "plaster") {
  const key = `${kind}:${material}:${hex}`;
  let tex = textureCache.get(key);
  if (!tex) {
    tex = makeSpeckleTexture(hex, kind, material);
...
function texturedMaterial(textureCache, hex, kind, segW, segH, roughness, material = "plaster") {
  const base = getBaseTexture(textureCache, hex, kind, material);
  ...
  const params = { map: tex, roughness: adjustRoughness(material, roughness) };
  if (kind === "wall") {
...
/**
 * @typedef {Pick<Room, "id"|"floor"|"gx"|"gy"|"gw"|"gh"|"wall_height"|"wall_color"|"wall_material">} BoundaryRoom
 * @typedef {Object} SharedBoundary
 * @property {{ room: BoundaryRoom, wall: "top"|"bottom"|"left"|"right" }} a
 * @property {{ room: BoundaryRoom, wall: "top"|"bottom"|"left"|"right" }} b
...
/**
 * @param {BoundaryRoom[]} rooms
 * @returns {SharedBoundary[]}
 */
...
/**
 * @param {SharedBoundary[]} boundaries
...
// ====== جدران مشتركة تشمل الغرف الحرة الشكل ======
// المسار المستطيل فوق بيقارن محاور متعامدة (x مع x، y مع y) — ما بينفع لضلع مايل بزاوية
// عشوائية. المسار هون عام: كل غرفة (حرة أو مستطيلة) بتتحوّل لقائمة قطع مستقيمة موجّهة،
// وأي ضلعين من غرفتين مختلفتين بينفحصوا كقطعتين: نفس الاستقامة + تراكب فعلي بالطول.
//
// **ليش مسار منفصل بدل استبدال المسار المستطيل**: المسار المستطيل مُختبَر وشغّال (بما فيه
// تمديد نصف السماكة عند الزوايا الحقيقية، وخصمه بجدول الكميات)، وإعادة كتابته إلها مخاطرة
// انحدار حقيقية بلا أي مكسب للمستخدم. فالمسار الجديد بيغطي **بس** الأزواج يلي على الأقل
// وحدة منهم غرفة حرة (حر↔حر وحر↔مستطيل)، ومستطيل↔مستطيل بيضل حصرياً للمسار القديم —
// فما في ولا حد بينبنى مرتين ولا بينخصم مرتين.
export const EDGE_COLLINEAR_EPS = 0.02; // بالمتر — أقصى انحراف عمودي بين ضلعين ليُعتبروا نفس الجدار

/**
 * مفتاح موحّد لضلع بخريطة الفترات المشتركة — جدار مستطيل مسمّى أو ضلع شكل حر مرقّم.
 * @param {string} roomId
 * @param {string|null|undefined} wall
 * @param {number|null|undefined} edgeIndex
 * @returns {string}
 */
export function wallRangeKey(roomId, wall, edgeIndex) {
  return edgeIndex != null ? `${roomId}:edge:${edgeIndex}` : `${roomId}:${wall}`;
}

/**
 * @typedef {Object} RoomEdge
 * @property {any} room
 * @property {"top"|"bottom"|"left"|"right"|null} wall
 * @property {number|null} edgeIndex
 * @property {Point} p1
 * @property {Point} p2
...
/**
 * كل أضلاع غرفة كقطع مستقيمة موجّهة، **بنفس اتجاه قياس `position` للفتحات عليها بالضبط**
 * (من p1 لـp2، نفس اصطلاح openingMarkPoints للحالتين) — فالفترة المحلية يلي بترجع من
 * التقاطع بتنقارن مباشرة بمواضع الأبواب والنوافذ المخزّنة بلا أي تحويل إضافي.
 * @param {any} room
 * @returns {RoomEdge[]}
 */
export function roomEdges(room) {
  if (room.points && room.points.length >= 3) {
    return room.points.map((p1, i) => {
      ...
      return { room, wall: null, edgeIndex: i, p1, p2, length: Math.hypot(p2.x - p1.x, p2.y - p1.y) };
    });
  }
  const { gx, gy, gw, gh } = room;
  return [
    { room, wall: "top", edgeIndex: null, p1: { x: gx, y: gy }, p2: { x: gx + gw, y: gy }, length: gw },
    { room, wall: "bottom", edgeIndex: null, p1: { x: gx, y: gy + gh }, p2: { x: gx + gw, y: gy + gh }, length: gw },
    { room, wall: "left", edgeIndex: null, p1: { x: gx, y: gy }, p2: { x: gx, y: gy + gh }, length: gh },
    { room, wall: "right", edgeIndex: null, p1: { x: gx + gw, y: gy }, p2: { x: gx + gw, y: gy + gh }, length: gh },
  ];
}

/**
 * هل ضلعان هما نفس الجدار فعلياً؟ الشرط: طرفا B على بُعد عمودي مهمل عن **خط** A (استقامة
 * وتطابق بضربة وحدة)، وبينهم طول تراكب يستاهل. بترجع الفترة المحلية على كل ضلع بوحداته
 * الخاصة، ونقطتي الجزء المشترك بالعالم، و`flipped` = الضلعان بيمشوا بعكس بعض (الحالة
 * الطبيعية لغرفتين متجاورتين، لأن كل مضلع بيلف حوالين نفسه).
 * بتاخد **قطعة** موصوفة بطرفيها وطولها بس (مو ضلع غرفة كامل): هي هندسة صرفة ما بتحتاج
 * تعرف الغرفة ولا رقم الضلع، والتوصيف الضيق بيخلي اختبارها ممكن بقطع مباشرة.
 * @param {{ p1: Point, p2: Point, length: number }} ea
 * @param {{ p1: Point, p2: Point, length: number }} eb
 * @returns {{ a: {start: number, end: number}, b: {start: number, end: number}, p1: Point, p2: Point, length: number, flipped: boolean } | null}
 */
export function edgeOverlap(ea, eb) {
  const la = ea.length, lb = eb.length;
  if (!(la > EDGE_COLLINEAR_EPS) || !(lb > EDGE_COLLINEAR_EPS)) return null;
  const ux = (ea.p2.x - ea.p1.x) / la, uy = (ea.p2.y - ea.p1.y) / la;

  const perp = (p) => Math.abs((p.x - ea.p1.x) * uy - (p.y - ea.p1.y) * ux);
  if (perp(eb.p1) > EDGE_COLLINEAR_EPS || perp(eb.p2) > EDGE_COLLINEAR_EPS) return null;

  const proj = (p) => (p.x - ea.p1.x) * ux + (p.y - ea.p1.y) * uy;
  const tb1 = proj(eb.p1), tb2 = proj(eb.p2);
  const start = Math.max(0, Math.min(tb1, tb2));
  const end = Math.min(la, Math.max(tb1, tb2));
  if (end - start <= OVERLAP_EPS) return null;

  // نفس الجزء بمعامل B — لو B ماشي بعكس A، المعامل بينعكس
  const flipped = tb2 < tb1;
  const toB = (t) => (flipped ? tb1 - t : t - tb1);
  const clampB = (v) => Math.min(lb, Math.max(0, v));
  const b1 = toB(start), b2 = toB(end);

  return {
    a: { start, end },
    b: { start: clampB(Math.min(b1, b2)), end: clampB(Math.max(b1, b2)) },
    p1: { x: ea.p1.x + ux * start, y: ea.p1.y + uy * start },
    p2: { x: ea.p1.x + ux * end, y: ea.p1.y + uy * end },
    length: end - start,
    flipped,
  };
}
...
/**
 * @typedef {Object} PolySharedSide
 * @property {any} room
 * @property {"top"|"bottom"|"left"|"right"|null} wall
 * @property {number|null} edgeIndex
 * @property {number} start الفترة المحلية بوحدات هاد الضلع
 * @property {number} end
 * @property {number} edgeLength طول الضلع الكامل — لازم لتحويل مواضع الفتحات
 *
 * @typedef {Object} PolySharedBoundary
 * @property {PolySharedSide} a
 * @property {PolySharedSide} b
 * @property {Point} p1
 * @property {Point} p2
 * @property {number} length
...
/**
 * كل الحدود المشتركة يلي على الأقل وحدة من غرفتيها حرة الشكل.
 * @param {any[]} rooms
 * @returns {PolySharedBoundary[]}
 */
export function computePolygonSharedBoundaries(rooms) {
  const isPoly = (r) => !!(r.points && r.points.length >= 3);
  const out = [];
  for (let i = 0; i < rooms.length; i++) {
    const a = rooms[i], af = a.floor ?? 0;
    for (let j = i + 1; j < rooms.length; j++) {
      const b = rooms[j];
      if ((b.floor ?? 0) !== af) continue;
      if (!isPoly(a) && !isPoly(b)) continue; // مستطيل↔مستطيل: للمسار القديم حصرياً
      roomEdges(a).forEach((ea) => {
        roomEdges(b).forEach((eb) => {
          const ov = edgeOverlap(ea, eb);
          if (!ov) return;
          out.push({
            a: { room: a, wall: ea.wall, edgeIndex: ea.edgeIndex, start: ov.a.start, end: ov.a.end, edgeLength: ea.length },
            b: { room: b, wall: eb.wall, edgeIndex: eb.edgeIndex, start: ov.b.start, end: ov.b.end, edgeLength: eb.length },
            p1: ov.p1, p2: ov.p2, length: ov.length, flipped: ov.flipped,
          });
        });
      });
    }
...
/**
 * بتضيف فترات الحدود الحرة لنفس خريطة `sharedWallRanges` (بتعدّل الخريطة المعطاة وبترجعها)
 * — هيك منطق "ابنِ الجزء المتبقي بس" ومنع النوافذ على جدار مشترك بيشتغلوا للحالتين عبر
 * نفس المسار بالضبط، بلا فرع ثاني بكل مكان.
 * @param {Map<string, {start: number, end: number}[]>} map
 * @param {PolySharedBoundary[]} boundaries
 * @returns {Map<string, {start: number, end: number}[]>}
 */
export function addPolygonSharedRanges(map, boundaries) {
  const raw = new Map();
  boundaries.forEach((bnd) => {
    [bnd.a, bnd.b].forEach((side) => {
      const key = wallRangeKey(side.room.id, side.wall, side.edgeIndex);
      if (!raw.has(key)) raw.set(key, []);
      raw.get(key).push({ start: side.start, end: side.end });
    });
  });
  raw.forEach((ranges, key) => {
    map.set(key, mergeLocalRanges([...(map.get(key) || []), ...ranges]));
  });
  return map;
}
...
/**
 * @param {import("./types").Opening} o
 * @param {number} wallLength
 * @param {number} wallHeight
 * @returns {{ start: number, width: number, sill: number, openH: number, isDoor: boolean }}
 */
export function openingRowToShape(o, wallLength, wallHeight) {
...
function buildWall(group, animState, textureCache, wallColorHex, wallMaterial, opts) {
  ...
  pillars.forEach((p) => {
    ...
    const mat = texturedMaterial(textureCache, wallColorHex, "wall", segLen, wallHeight, 0.85, wallMaterial);
  ...
  sorted.forEach((o) => {
    ...
    if (o.sill > 0.02) {
      const sillMat = texturedMaterial(textureCache, wallColorHex, "wall", o.width, o.sill, 0.85, wallMaterial);
    ...
    if (headerH > 0.02) {
      const headerMat = texturedMaterial(textureCache, wallColorHex, "wall", o.width, headerH, 0.85, wallMaterial);
...
function buildRoomRoof(group, animState, textureCache, room, wallHeight, floorBaseY, wallColorHex, wallMaterial, cx, cz, withParapet, needsSlabForFloorAbove) {
  if (!withParapet && !needsSlabForFloorAbove) return;

  // سقف جملوني/hip بس للحالة "الحقيقية": الغرفة بأعلى طابق (ما في طابق فوقها يحتاج بلاطة أرضية)
  ...
  if (withParapet && !needsSlabForFloorAbove && !room.points) {
    if (room.roof_type === "gable") {
      buildGableRoof(group, animState, textureCache, room, wallHeight, floorBaseY, wallColorHex, wallMaterial, cx, cz);
      return;
    }
    if (room.roof_type === "hip") {
      buildHipRoof(group, animState, textureCache, room, wallHeight, floorBaseY, wallColorHex, cx, cz);
      return;
    }
  }
...
function buildGableRoof(group, animState, textureCache, room, wallHeight, floorBaseY, wallColorHex, wallMaterial, cx, cz) {
  ...
  const gableMat = texturedMaterial(textureCache, wallColorHex, "wall", wallSpan, ridgeHeight, 0.85, wallMaterial);
  [-1, 1].forEach((endSide) => {
...
// هندسة سطح "أربعة ميول" (hip) — 4 أوجه مستوية حقيقية (بلا تقريب صناديق): وجهان شبه منحرف
// (الضلعان الطويلان، من حافة الجدار الطويلة لطرفي الحافة/ridge) ووجهان مثلث (الضلعان
// القصيران، من حافة الجدار القصيرة لطرف الحافة الأقرب). كل الأوجه بميل ROOF_PITCH نفسه —
// هندسة "hip roof" المعيارية: طول الحافة = البعد الطويل - البعد القصير (تلقائياً بيصير صفر
// لغرفة مربعة = سطح هرمي بأربعة أوجه مثلثة بس، حالة خاصة بلا كود إضافي). المحور الطويل دايماً
// x هون (استدعاء buildHipRoof بيدوّر الهندسة كاملة 90° لو الحافة على محور z — دوران كامل
// بمضاعف 90° عبر geometry.rotateY، مو زاوية جزئية، فبلا أي خطر تقريب/إشارة خاطئة).
// الرؤوس مبنية بإحداثيات cx/cz المطلقة مباشرة (بعكس buildFlatSlabGeometry يلي بيرجع للمركز
// بعدين addGrowSlab بيحطها بموضع 0,0 — هون أسهل نبني الإحداثيات المطلقة مباشرة بما إنه
// مستطيل بسيط بلا حاجة لـ THREE.Shape)، ومزاحة عمودياً بمقدار ridgeHeight/2 (نفس مبدأ
// buildFlatSlabGeometry: نمركز الهندسة حوالين الصفر المحلي عشان تتوافق مع animState.meshes)
function buildHipRoofGeometry(cx, cz, hw, hd, ridgeHeight) {
  const yShift = ridgeHeight / 2;
  const A = [cx - hw, -yShift, cz - hd];
  const B = [cx + hw, -yShift, cz - hd];
  const C = [cx + hw, -yShift, cz + hd];
  const D = [cx - hw, -yShift, cz + hd];
  const R1 = [cx - (hw - hd), ridgeHeight - yShift, cz];
  const R2 = [cx + (hw - hd), ridgeHeight - yShift, cz];

  const tris = [
    D, C, R2, D, R2, R1, // وجه شبه منحرف أمامي (z = cz+hd)
    B, A, R1, B, R1, R2, // وجه شبه منحرف خلفي (z = cz-hd)
    A, D, R1,            // وجه مثلث جانب سالب-x
    C, B, R2,            // وجه مثلث جانب موجب-x
  ];
...
function buildHipRoof(group, animState, textureCache, room, wallHeight, floorBaseY, wallColorHex, cx, cz) {
  const ridgeAlongX = room.gw >= room.gh;
  const span = (ridgeAlongX ? room.gh : room.gw) + ROOF_OVERHANG * 2; // القصير
  const ridgeFull = (ridgeAlongX ? room.gw : room.gh) + ROOF_OVERHANG * 2; // الطويل
  const halfSpan = span / 2;
  const halfRidgeFull = ridgeFull / 2;
  const ridgeHeight = halfSpan * Math.tan(ROOF_PITCH);

  const geometry = ridgeAlongX
    ? buildHipRoofGeometry(cx, cz, halfRidgeFull, halfSpan, ridgeHeight)
    : buildHipRoofGeometry(0, 0, halfRidgeFull, halfSpan, ridgeHeight);
  if (!ridgeAlongX) {
    geometry.rotateY(Math.PI / 2); // يبدّل المحور الطويل من x لـ z بدوران كامل 90° (بلا cx/cz بعد — نضيفهم بترجمة مباشرة تحت)
    geometry.translate(cx, 0, cz);
  }

  const roofColor = new THREE.Color(wallColorHex).multiplyScalar(0.72);
  const roofHex = `#${roofColor.getHexString()}`;
  const roofMat = texturedMaterial(textureCache, roofHex, "roof", ridgeFull, span, 0.95);
  roofMat.side = THREE.DoubleSide;

  addGrowSlab(group, animState, geometry, roofMat, floorBaseY + wallHeight, ridgeHeight, ROOF_DELAY);
}
 ...
 * @param {any} animState
 * @param {Map<string, THREE.Texture>} textureCache
 * @param {import("./types").FurnitureItem} item
 * @param {number} worldX
 * @param {number} worldZ
 * @param {number} floorBaseY
 */
...
/**
 * @param {number} floorToFloorHeight
 * @returns {{ w: number, d: number, numSteps: number }}
 */
 ...
 * @param {number} worldZ
 * @param {number} floorBaseY
 * @param {string} wallColorHex - قيمة لون CSS (متوافقة مباشرة مع THREE.Color)، مو رقم hex فعلي
 */
...
function buildAngledWallSegment(group, animState, textureCache, wallColorHex, wallMaterial, p1, p2, wallHeight, floorBaseY, center, openings = []) {
...
// حوالين كل ضلع بس لو has_roof مفعّل (صلبة دايماً، بلا فتحات). الأثاث يُبنى منفصل (نقطة
// داخل مضلع لحالها بواجهة المخطط 2D، بلا حاجة لتغيير هون).
// الأجزاء المشتركة مع غرفة جارة بتُستثنى من هون وبتتبنى مرة وحدة بس بحلقة الحدود
// برَبْعها بـrebuildGroup — بلا فترات مشتركة (الحالة الغالبة) الحساب بيرجع الضلع كامل
// حرفياً (p1→p2)، فالسلوك بيضل مطابق تماماً لما كان قبل دمج الجدران
function buildPolygonRoomShell(group, animState, textureCache, room, wallHeight, floorBaseY, wallColorHex, wallMaterial, center, needsSlabForFloorAbove, sharedRanges, hideRoofs = false) {
  const points = room.points;
  if (!points || points.length < 3) return;

  const floorGeo = buildFlatSlabGeometry(points, 0.06, center);
  const floorMat = texturedMaterial(textureCache, room.color, "floor", 4, 4, 0.9, room.floor_material);
  ...
  for (let i = 0; i < points.length; i++) {
    ...
    const shared = sharedRanges?.get(wallRangeKey(room.id, null, i)) || [];
    complementRanges(shared, edgeLength).forEach((seg) => {
      const at = (d) => ({ x: p1.x + ((p2.x - p1.x) * d) / edgeLength, y: p1.y + ((p2.y - p1.y) * d) / edgeLength });
      const segOpenings = edgeOpenings
        .filter((o) => o.start >= seg.start - 0.02 && o.start + o.width <= seg.end + 0.02)
        .map((o) => ({ ...o, start: o.start - seg.start }));
      buildAngledWallSegment(group, animState, textureCache, wallColorHex, wallMaterial, at(seg.start), at(seg.end), wallHeight, floorBaseY, center, segOpenings);
    });
  }
  ...
  if (hideRoofs || (!room.has_roof && !needsSlabForFloorAbove)) return;
  ...
  if (room.has_roof) {
    const parapetBottom = floorBaseY + wallHeight + ROOF_T;
    for (let i = 0; i < points.length; i++) {
      buildAngledWallSegment(group, animState, textureCache, roofHex, wallMaterial, points[i], points[(i + 1) % points.length], PARAPET_H, parapetBottom, center);
    }
  }
}
 ...
 * @param {any} animState
 * @param {Map<string, THREE.Texture>} textureCache
 * @param {string} [wallMaterialDefault]
 * @param {{ hideRoofs?: boolean, floorGap?: number }} [options] وضع "بيت الدمية": بلا أسقف
 *   (بتشوف جوا كل غرفة من فوق) وبفراغ بين الطوابق. القيم الافتراضية = البناء الطبيعي تماماً،
 *   فكل نداء قديم بيضل يعطي نفس النتيجة حرفياً
 */
export function rebuildGroup(group, rooms, stairs, wallHeight, wallColor, center, animState, textureCache, wallMaterialDefault = "plaster", options = {}) {
  const hideRoofs = !!options.hideRoofs;
  const floorGap = options.floorGap ?? 0;
  ...
  const polyBoundaries = computePolygonSharedBoundaries(rooms);
  // خريطة وحدة للمسارين — الجزء المستطيل من حد حر↔مستطيل بينخصم من جدار المستطيل عبر
  // نفس منطق complementRanges الموجود، بلا أي فرع إضافي بحلقة المستطيلات
  const sharedRanges = addPolygonSharedRanges(sharedWallRanges(boundaries), polyBoundaries);
  const stairsMinTopFloor = Math.max(0, ...(stairs || []).map((s) => (s.floor ?? 0) + 1));
  const floorBaseYMap = computeFloorBaseYMap(rooms, wallHeight, stairsMinTopFloor, floorGap);
  ...
  rooms.forEach((r) => {
    ...
    const roomWallMaterial = r.wall_material ?? wallMaterialDefault;
    const needsSlabForFloorAbove = roomHasOverlapAbove(r, rooms);

    if (r.points) {
      buildPolygonRoomShell(group, animState, textureCache, r, roomWallHeight, floorBaseY, roomWallColor, roomWallMaterial, center, needsSlabForFloorAbove, sharedRanges, hideRoofs);
      return;
    }
    ...
    const floorMat = texturedMaterial(textureCache, r.color, "floor", r.gw, r.gh, 0.9, r.floor_material);
    ...
    WALL_SIDES.forEach(({ wall, axis, doorFallback }) => {
      ...
      remainder.forEach((seg) => {
        ...
        buildWall(group, animState, textureCache, roomWallColor, roomWallMaterial, {
          axis, u0, fixed: fixedWorld, wallLength, wallHeight: roomWallHeight, floorBaseY, openings: segOpenings,
        });
      });
    });

    buildRoomRoof(group, animState, textureCache, r, roomWallHeight, floorBaseY, roomWallColor, roomWallMaterial, cx, cz, !hideRoofs && !!r.has_roof, !hideRoofs && needsSlabForFloorAbove);
  ...
  boundaries.forEach((bnd) => {
    ...
    const roomWallMaterial = ra.wall_material ?? wallMaterialDefault;
    ...
    buildWall(group, animState, textureCache, roomWallColor, roomWallMaterial, { axis, u0, fixed, wallLength, wallHeight: roomWallHeight, floorBaseY, openings: merged });
  });

  // جدار واحد مشترك لكل جزء متراكب بين غرفة حرة وجارتها (حرة أو مستطيلة) — نفس قاعدة
  // المسار المستطيل: أبواب الغرفتين تضل ظاهرة، والنوافذ ما بتنوجد أصلاً على جدار مشترك
  // (hitTestWalls بتمنع وضعها عليه)
  polyBoundaries.forEach((bnd) => {
    const ra = bnd.a.room;
    const floorBaseY = floorBaseYMap.get(ra.floor ?? 0);
    const roomWallHeight = ra.wall_height ?? wallHeight;
    const roomWallColor = ra.wall_color ?? wallColor;
    ...
    // موضع الباب بإحداثي الجزء المشترك (الصفر عند bnd.p1): جانب A ماشي بنفس اتجاهه،
    // وجانب B ممكن يكون معاكس — غرفتان متجاورتان بيلفّوا بعكس بعض على حدهم المشترك
    const doorsOn = (side, isB) =>
      (side.room.openings || [])
        .filter((o) => (side.edgeIndex != null ? o.edge_index === side.edgeIndex : o.wall === side.wall) && o.kind === "door")
        .map((o) => openingRowToShape(o, side.edgeLength, roomWallHeight))
        .filter((sh) => sh.start >= side.start - 0.02 && sh.start + sh.width <= side.end + 0.02)
        .map((sh) => ({
          ...sh,
          start: isB && bnd.flipped ? side.end - (sh.start + sh.width) : sh.start - side.start,
        }));

    const merged = dedupeDoorShapes([...doorsOn(bnd.a, false), ...doorsOn(bnd.b, true)]);
    buildAngledWallSegment(group, animState, textureCache, roomWallColor, roomWallMaterial, bnd.p1, bnd.p2, roomWallHeight, floorBaseY, center, merged);
  });
