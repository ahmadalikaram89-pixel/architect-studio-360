...
import { roomArea, computeSharedBoundaries, computePolygonSharedBoundaries } from "./build3d";
...
// طول الجدران الصافي بطابق معيّن — مجموع محيط كل الغرف ناقص طول الأجزاء المشتركة (كل
// جدار مشترك بين غرفتين متلاصقتين محسوب مرة وحدة بس، مو مرتين). الحدود المستطيلة والحرة
// بتنحسب بمسارين منفصلين ما بيتقاطعوا (computePolygonSharedBoundaries بتتخطى أزواج
// مستطيل↔مستطيل عمداً)، فما في خصم مضاعف لنفس الجدار.
function floorWallLength(floorRooms) {
  ...
  const polySharedSum = computePolygonSharedBoundaries(floorRooms).reduce((s, b) => s + b.length, 0);

  return (rawPerimeterSum - sharedSum) + polyPerimeterSum - polySharedSum;
}
