...
import { DOOR_W, WIN_W, FURNITURE_KINDS, computeFloorBaseYMap, stairFootprint, roomArea, wallRangeKey } from "./build3d";

/** @typedef {import("./types").Room} Room */
/** @typedef {import("./types").Point} Point */
/** @typedef {import("./types").Opening} Opening */
/** @typedef {import("./types").FurnitureItem} FurnitureItem */
...
// ====== زوم/تحريك المخطط 2D ======
// الزوم مطبَّق كقياس CSS للكانفاس (عرض/ارتفاع بالبكسل = أبعاد المخطط × الزوم)، والتحريك
// (pan) هو تمرير الحاوية الأب العادي. الرسم نفسه بيضل بإحداثيات المخطط الخام (متر × PPM)
// — الزوم بينضاف كمصفوفة تحويل وحدة وحدة على السياق (setTransform) وقت الرسم، فما في ولا
// سطر رسم لازم يعرف شي عن الزوم، والنتيجة بتضل حادّة بأي تكبير (مو صورة مكبّرة ضبابية).
export const MIN_ZOOM = 0.25;
export const MAX_ZOOM = 6;
export const ZOOM_STEP = 1.25; // نسبة كل ضغطة زر تكبير/تصغير

/**
 * حصر مستوى الزوم ضمن المدى المسموح (وحماية من قيم غير رقمية).
 * @param {number} z
 * @returns {number}
 */
...
/**
 * زوم "الملاءمة" — أكبر تكبير بيخلي المخطط كامل يدخل ضمن الحاوية المتوفرة.
 * ما بيتجاوز 1 عمداً: هيك عرض البداية بيضل مطابق تماماً للسلوك القديم (`maxWidth: 100%`
 * — تصغير عند اللزوم بس، بلا تكبير تلقائي لمخطط صغير على شاشة كبيرة).
 * @param {number} boxW عرض الحاوية بالبكسل
 * @param {number} boxH ارتفاع الحاوية بالبكسل
 * @param {number} gridW عرض الأرض بالمتر
 * @param {number} gridH عمق الأرض بالمتر
 * @param {number} [pad] هامش بالبكسل بيتترك حوالين المخطط
 * @returns {number}
 */
export function computeFitZoom(boxW, boxH, gridW, gridH, pad = 32) {
  const planW = gridW * PPM, planH = gridH * PPM;
  if (!(boxW > 0) || !(boxH > 0) || !(planW > 0) || !(planH > 0)) return 1;
  const ratio = Math.min((boxW - pad) / planW, (boxH - pad) / planH);
  return clampZoom(Math.min(1, ratio));
}

/**
 * تحويل إحداثيات مؤشر (بكسل شاشة) لإحداثيات المخطط بالمتر.
 * بيعتمد على **نسبة** موقع المؤشر داخل المستطيل المقاس فعلياً للكانفاس، فهو مستقل تماماً
 * عن مستوى الزوم وعن كثافة بكسل الشاشة (devicePixelRatio) — ما بيحتاج يعرف ولا وحدة منهم.
 * @param {number} clientX
 * @param {number} clientY
 * @param {{left: number, top: number, width: number, height: number}} rect ناتج getBoundingClientRect للكانفاس
 * @param {number} gridW عرض الأرض بالمتر
 * @param {number} gridH عمق الأرض بالمتر
 * @returns {Point}
 */
export function clientToPlanMeters(clientX, clientY, rect, gridW, gridH) {
  if (!rect || !(rect.width > 0) || !(rect.height > 0)) return { x: 0, y: 0 };
  return {
    x: clamp(((clientX - rect.left) / rect.width) * gridW, 0, gridW),
    y: clamp(((clientY - rect.top) / rect.height) * gridH, 0, gridH),
  };
}
 ...
 * @param {number} cx
 * @param {number} cy
 * @param {number} openingWidth
 * @param {"door"|"window"} kind
 * @param {Map<string, {start: number, end: number}[]>} [sharedRanges]
 * @param {string} [excludeOpeningId]
 * @returns {({ roomId: string, edgeIndex: number, position: number } | { roomId: string, wall: string, position: number }) | null}
 */
  ...
  rooms.forEach((r) => {
    if (r.points) {
      for (let i = 0; i < r.points.length; i++) {
        ...
        // نافذة على جزء مشترك مع غرفة جارة ما إلها معنى (بتطل على غرفة تانية) — نفس
        // قاعدة الجدران المستطيلة المشتركة بالضبط، وعبر نفس الخريطة
        if (kind === "window") {
          const ranges = sharedRanges?.get(wallRangeKey(r.id, null, i));
          if (ranges?.some((rg) => rawPosition >= rg.start - 0.001 && rawPosition <= rg.end + 0.001)) continue;
        }
    ...
    walls.forEach((w) => {
      ...
      if (kind === "window") {
        const ranges = sharedRanges?.get(wallRangeKey(r.id, w.wall, null));
 ...
 * @param {number} cx
 * @param {number} cy
 * @returns {{ dist: number, roomId: string, id: string, wall: string|undefined, edgeIndex: number|undefined, position: number, kind: string } | null}
 */
...
/**
 * @param {Pick<Room, "points"|"gx"|"gy"|"gw"|"gh">} room
 * @param {Opening & { edgeIndex?: number }} opening
 * @param {number} width
...
// الغرف الحرة (points) لسا برّا النطاق هون تحديداً: ما إلها بُعدان بسيطان (عرض × عمق)
// تنكتب بخطي قياس زي المستطيل، بيلزمها قياس لكل ضلع بزاويته — بس مساحتها الإجمالية ظاهرة
// عبر roomArea (وهاد قيد خاص بالقياسات لحالها، مو قيد عام على الغرف الحرة)
/**
 * @param {CanvasRenderingContext2D} ctx
 * @param {Pick<Room, "points"|"gx"|"gy"|"gw"|"gh">} room
 * @param {{ line: string, text: string }} colors
 */
...
/**
 * @param {{ points?: Point[], gx?: number, gy?: number, gw?: number, gh?: number }} room
 * @returns {Point}
 */
...
/**
 * @param {Pick<Room, "id"|"gx"|"gy"|"furniture">[]} rooms
 * @param {number} cx
 * @param {number} cy
 * @returns {{ dist: number, roomId: string, id: string, kind: string, x: number, y: number, rotation: number } | null}
 */
...
/**
 * @param {Pick<Room, "id"|"gx"|"gy"|"gw"|"gh"|"points"|"furniture">[]} rooms
 * @param {number} cx
 * @param {number} cy
 * @param {string} kind
 * @param {string} [excludeId]
 * @returns {{ roomId: string, x: number, y: number } | null}
 */
 ...
 * @param {number} floor
 * @param {number} projectWallHeight
 * @returns {number}
 */
...
/**
 * @param {Stair[]} stairsForFloor
 * @param {Pick<Room, "floor"|"wall_height">[]} rooms
 * @param {number} projectWallHeight
 * @param {number} cx
 * @param {number} cy
 * @returns {{ dist: number, id: string, floor: number, x: number, y: number, rotation: number } | null}
 */
...
/**
 * @param {number} topAnchorM
 * @param {number} bottomAnchorM
 * @param {number} gridH
 * @returns {{ pct: number, flip: boolean }}
 */
